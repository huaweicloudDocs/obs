...
|**元素名称**|**描述**|**是否必选**|
|--|--|--|
|StorageClass|指定桶默认存储类型。类型：String取值范围： STANDARD | WARM  | COLD  | DEEP_ARCHIVE存储类型有4种：STANDARD（标准存储）、WARM（低频访问存储）、COLD（归档存储）、DEEP_ARCHIVE（深度归档存储）。因此这里可配置的值有：STANDARD、WARM和COLD、DEEP_ARCHIVE（受限公测）|是|
