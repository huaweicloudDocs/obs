...
桶用于存放对象的容器，是OBS中最高等级的命名空间。每个对象都存放在一个桶中。例如，如果名为“picture.jpg”的对象存放在“photo”桶中，则可使用 URL：http://photo.obs.cn-north-1.myhuaweicloud.com/picture.jpg对该对象进行寻址。您可以根据业务需要和网络位置来选择特定区域创桶。有关创桶详细信息，请参阅[创建桶](创建桶.md)。
